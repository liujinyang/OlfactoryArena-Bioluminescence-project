...
if (isnumeric(s) || islogical(s))
  ...
  if (min(dim)<1 || length(dim)>2) % if 1D or 3D array
    ...
    str=mat2str(s);           % convert matrix to a string
    str=regexprep(str,';',';\n');
  end
  % mark logical arrays with [] so the xml_read recognizes them as MATLAB
  % objects instead of strings
  if (islogical(s) && isscalar(s)), 
    str = ['[' str ']']; 
  end 
elseif iscell(s)              % Convert to MATLAB command notation
  if ndims(s)>2, s=s(:); end  % can't handle cell arrays with dimention > 2
  [nr nc] = size(s);
  s2 = s;
  for i=1:length(s(:))
    str = var2str(s{i}, PreserveSpace);
    if (ischar(s{i})), s{i} = ['''' s{i} '''']; else s{i}=str; end
    s2{i} = [s{i} ','];
  end
  for r = 1:nr, s2{r,nc} = [s{r,nc} ';']; end
  s2 = s2.';
  str = ['{' s2{:} '}'];
elseif isstruct(s)
  str='';
